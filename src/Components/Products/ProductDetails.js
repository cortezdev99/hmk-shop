...
export default (props) => {
  const { products, setProducts, setIsCartOpen } = useContext(CartContext);
  ...
  if (propsData) {
    const { title, images, sizes, details } =
      propsData.productDetailsProps.product;
    ...
    const handleAddToCart = () => {
      ...
      if (duplicate.length < 1) {
        ...
        state.push([
          { product: propsData.productDetailsProps.product },
          { size: size },
          { color: color },
      ...
      <div className="product-details-container">
        <div className="product-details-left-column-wrapper">
          <img
            ...
            src={
              Object.values(
                Object.values(images[activeImageSet])[0][activeImage]
              )[0]
            }
          ...
          <ProductDetailsGallerySlider
            images={images}
            handleGalleryImageClick={(imageSetIdx) =>
              handleGalleryImageClick(imageSetIdx)
            }
        ...
        <div className="product-details-right-column-wrapper">
          <div className="product-details-product-title-wrapper">{title}</div>

          <div className="product-details-product-colors-container">
            <div className="product-details-product-header">Color</div>

            <div className="product-details-product-colors-wrapper">
              {images.map((imageSet, imageSetIdx) => {
                const styles =
                  imageSetIdx === activeColor
                    ? { border: "1px solid #1d1d1d" }
                    : {};
                return (
                  <div
                    className="product-details-product-color-wrapper"
                    key={imageSetIdx}
                  >
                    <img
          ...
          <div className="product-details-product-sizes-container">
            <div className="product-details-product-header">Size</div>

            <div className="product-details-product-sizes-wrapper">
              {sizes.map((size, sizeIdx) => {
                const styles =
                  sizeIdx === activeSize ? { border: "1px solid #1d1d1d" } : {};
                return (
                  <div
                    style={styles}
          ...
          <div className="product-details-product-details-container">
            <div className="product-details-product-header">Details</div>

            <div className="product-details-product-details-wrapper">
              {details.map((detail, detailIdx) => {
                return (
                  <li
                    className="product-details-product-detail-wrapper"
                    key={detailIdx}
                  >
                    {detail}
                  </li>
          ...
          <div className="product-details-product-cart-btn-container">
            <button
              ...
              className="product-details-product-cart-btn"
            >
              Add to Cart
            </button>
          </div>
