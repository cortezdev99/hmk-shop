...
export default (props) => {
  ...
  const handleTranslatingImages = (direction) => {
    if (direction === 'Left' && activeGalleryIdx !== 2 && window.document.body.clientWidth > 640) {
      setActiveGalleryIdx(activeGalleryIdx - 1)
      return setTranslatePxls(translatePxls + 60)
    } else if (direction === 'Right' && activeGalleryIdx < props.product.images.length - 1 && window.document.body.clientWidth > 640) {
      setActiveGalleryIdx(activeGalleryIdx + 1)
      setTranslatePxls(translatePxls - 60)
    } else if (direction === 'Left' && activeGalleryIdx !== 2 && window.document.body.clientWidth <= 640 && window.document.body.clientWidth > 375) {
      setActiveGalleryIdx(activeGalleryIdx - 1)
      return setTranslatePxls(translatePxls + 55)
    } else if (direction === 'Right' && activeGalleryIdx < props.product.images.length && window.document.body.clientWidth <= 640 && window.document.body.clientWidth > 375) {
      setActiveGalleryIdx(activeGalleryIdx + 1)
      setTranslatePxls(translatePxls - 55)
    } else if (direction === 'Left' && activeGalleryIdx !== 2 && window.document.body.clientWidth <= 375 && window.document.body.clientWidth > 325) {
      setActiveGalleryIdx(activeGalleryIdx - 1)
      return setTranslatePxls(translatePxls + 46.5)
    } else if (direction === 'Right' && activeGalleryIdx < props.product.images.length && window.document.body.clientWidth <= 375  && window.document.body.clientWidth > 325) {
      setActiveGalleryIdx(activeGalleryIdx + 1)
      setTranslatePxls(translatePxls - 46.5)
    } else if (direction === 'Left' && activeGalleryIdx !== 2 && window.document.body.clientWidth <= 325) {
      setActiveGalleryIdx(activeGalleryIdx - 1)
      return setTranslatePxls(translatePxls + 38.19)
    } else if (direction === 'Right' && activeGalleryIdx < props.product.images.length && window.document.body.clientWidth <= 325) {
      setActiveGalleryIdx(activeGalleryIdx + 1)
