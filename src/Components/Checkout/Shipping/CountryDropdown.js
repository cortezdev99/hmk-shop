...
export default (props) => {
  ...
  const handleCollapsableContent = () => {
    if (el !== null && el3 !== null) {
      if (props.addShippingMaxHeight) {
        if (
          el.classList.contains("country-dropdown-collapsable-content-showing")
        ) {
          props.setAddShippingMaxHeight(props.addShippingMaxHeight - 180);
        } else {
          props.setAddShippingMaxHeight(props.addShippingMaxHeight + 180);
  ...
  return (
    <div
      style={{
        ...
        border: `${
          props.noRegionErr && props.region.length === 0
            ? "1px solid #FF0000"
            : "1px solid #CCC"
        }`,
        background: "#fbfbfb",
    ...
    >
      <div
        style={{
          ...
          padding: "0 7px",
          boxShadow:
            "-157px 5px 25px -14px rgba(50, 50, 50, 0.75), 147px 5px 25px -14px rgba(50, 50, 50, 0.75)",
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          cursor: "pointer",
        }}
        onClick={handleCollapsableContent}
      >
        <div
          style={{
            paddingRight: "10px",
        ...
        >
          {props.region.length === 0 ? (
            ...
            <span style={{ color: "#1d1d1d" }}>{props.region[0]}</span>
          )}
        </div>
